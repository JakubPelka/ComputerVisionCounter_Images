# ui_panels.py — main window layout (source first, then paths, quality, AOI, viz, classes)
...
def build_main_ui(app):
    ...
    # ---- Input source (Files/Folder vs Camera) — placed FIRST ----
    srcf = tk.LabelFrame(root, text="Input source")
    srcf.pack(fill="x", pady=(0, 6))

    def _src_toggle(*_):
        # enable/disable camera widgets; paths remain available (as in video_counter)
        try:
            cam_ent.config(state=("normal" if app.src_mode.get() == "camera" else "disabled"))
            cam_btn.config(state=("normal" if app.src_mode.get() == "camera" else "disabled"))
        except Exception:
            pass

    tk.Radiobutton(
        srcf, text="Files / Folder", variable=app.src_mode, value="files",
        command=_src_toggle
    ).pack(side="left", padx=6)

    tk.Radiobutton(
        srcf, text="Camera", variable=app.src_mode, value="camera",
        command=_src_toggle
    ).pack(side="left", padx=6)

    tk.Label(srcf, text="Index:").pack(side="left")
    cam_ent = tk.Entry(srcf, width=4, textvariable=app.cam_index)
    cam_ent.pack(side="left", padx=(0, 6))

    cam_btn = tk.Button(srcf, text="Open Camera…", command=app._open_camera_capture)
    cam_btn.pack(side="left", padx=6)

    _src_toggle()

    # ---- Paths ----
    paths = tk.LabelFrame(root, text="Paths")
    paths.pack(fill="x", pady=(0,6))

    _row_browse(paths, "Input folder (images):", app.input_dir, app.browse_input, is_dir=True)

    files = tk.Frame(paths)
    files.pack(fill="x", pady=2)
    tk.Button(files, text="Pick images…", command=app.browse_files).pack(side="left")
    app.files_label = tk.Label(files, text="— no files selected —")
    app.files_label.pack(side="left", padx=8)
    ...
    # ---- Quality (3 presets) ----
    qf = tk.LabelFrame(root, text="Quality presets (1 Fast • 2 Balanced • 3 Ultra)")
    ...
    app.preset_label = tk.Label(left, text="")
    app.preset_label.pack(side="left", padx=6)
    app._update_preset_label()

    tk.Button(qf, text="Advanced…", command=app.open_advanced).pack(side="right", padx=6, pady=2)

    # ---- AOI ----
    aoi = tk.LabelFrame(root, text="Areas of Interest (AOI)")
    aoi.pack(fill="x", pady=(0,6))

    tk.Checkbutton(aoi, text="Require AOI for every image", variable=app.require_aoi_all).pack(anchor="w")

    tk.Checkbutton(
        aoi,
        text="Use AOI (draw polygons — finish with Ctrl+Enter, undo vertex: Ctrl+Backspace)",
        variable=app.use_aoi,
        command=app._on_toggle_use_aoi
    ).pack(anchor="w")

    m = tk.Frame(aoi)
    m.pack(fill="x", pady=(4,2))
    ...
    act = tk.Frame(aoi)
    act.pack(fill="x", pady=(4,2))
    tk.Button(act, text="Open AOI editor…", command=app._open_aoi_editor).pack(side="left")

    # ---- Visualization ----
    vis = tk.LabelFrame(root, text="Visualization")
    vis.pack(fill="x", pady=(0,6))
    ...
    cl = tk.LabelFrame(root, text="Classes (load weights to populate)")
    cl.pack(fill="both", expand=True, pady=(0,6))
    ...
    app.btn_start = tk.Button(rf, text="START", command=app.start)
    app.btn_start.pack(side="left")
    app.btn_abort = tk.Button(rf, text="ABORT", command=app.abort, state="disabled")
    app.btn_abort.pack(side="left", padx=(8,0))
    ...
    app.log = tk.Text(logf, height=10)
